...
- `hado charge`: evidence の参照（例: coverage artifact の adapter/path）を manifest にマージする
...
`hado charge` の `--coverage-input` は manifest の `evidence.coverage.inputs` へ不足分をマージします。既存値は置換しません。
