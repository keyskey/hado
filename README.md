...
**設計上の CLI 体系:** 将来的には `hado target`（対話で manifest に service / standard を書き戻す照準）→ `hado charge`（manifest のメタデータから未充足 evidence を自動補完する充填）→ `hado fire`（判定のみ・デプロイはしない）の 3 段階に分け、`evaluate` は一括モードまたは `fire` の別名に寄せる想定です。詳細は [docs/overview.md](docs/overview.md) と [docs/architecture.md](docs/architecture.md) を参照してください。
