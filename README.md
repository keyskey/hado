...
`hado charge` は **Readiness Standard と manifest** を読み、`internal/charge` で **plan → gap → apply** を実行します。既定は dry-run（`--apply` なし）で、coverage の不足だけを報告します。`--apply` と `--preset go-gobce`（既定）では、manifest ディレクトリで `go test ./... -coverprofile=coverage.out` と `gobce analyze ...` を実行し、`evidence.coverage.inputs` に `gobce-json` を書き込みます（`gobce` が PATH に必要）。

```bash
./bin/hado charge --manifest hado.yaml --output text
./bin/hado charge --manifest hado.yaml --apply
```

`--standard` を省略すると、manifest の `standard.id` から `standards/<id>.yaml` を探します（`--standards-dir` でディレクトリを変えられます）。
...
**設計上の CLI 体系:** `hado target` で manifest に service / standard を記録し、`hado charge` で coverage evidence を plan/gap/apply します。将来的には `hado fire`（判定のみ・デプロイはしない）へ分ける想定です。いまは `hado evaluate` が判定を一括で行います。詳細は [docs/overview.md](docs/overview.md) と [docs/architecture.md](docs/architecture.md) を参照してください。
