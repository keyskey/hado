...
```text
Test readiness
  unit test, integration test, C0 coverage, C1 branch coverage,
...
**CLI フェーズとの対応（設計）:** **target** で「どの standard のどの gate に対応するか」を確定し、manifest に **evidence のプレースホルダー**を載せる。**charge** で evidence を揃えて正規化し、**fire** で照合と decision まで行う。現行の `hado evaluate` は **charge に相当する処理と fire を一括**で実行している。詳細は [概要](overview.md) と [アーキテクチャ](architecture.md) を参照する。
  ...
  "failedGates": [
    {
      "id": "test.c1_coverage",
      "severity": "major",
      "message": "C1 coverage is 68.4%, below the required 70%."
    },
    {
      "id": "operations.runbook_exists",
      "severity": "critical",
...
Blocking:
- C1 coverage is 68.4%, required 70%
...
```text
一時的に C1 が基準未満だが、期限付きで release を許可する。
