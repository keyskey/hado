...
```text
プロダクトの世界観・由来（宇宙戦艦ヤマトと波動砲）は overview や README で正面から説明してよい。
CLI のサブコマンド名・ゲート ID・ユーザー向けメッセージは、説明なしで意味が伝わる短い語を優先する。
物語の比喩は設計の動機の説明に使い、仕様の定義だけが比喩に依存しないようにする。
```
...
```text
「波動砲を撃てる状態」＝本番に出しても運用に耐えられる状態、という連想で Target などの語を検討した。
...
`hado fire`（設計上の判定コマンド。移行期間は `hado evaluate` が同じ役割）が `--manifest hado.yaml` から coverage input を読める。
...
[概要](overview.md) で固定した **target → charge → fire** を実装するときの未決はここに追記する。

**方針（マニフェスト中心）:** `target` は対話で manifest に書き戻し、`charge` は既存の manifest メタデータと target の回答から **未充足 evidence を自動補完**する。**評価の正は manifest** とし、`.hado/context.json` のような中間状態ファイルを必須にしない。

- **`target`:** プロンプトの質問項目（最小限は service 識別子と standard 参照か）、非対話モード（フラグのみ）との両立。
- **`charge`:** どの gate 欠損をどの module が埋めるかの宣言の置き場、CI で `go test` 等を別ステップに置くか `charge` に内包するか、manifest への書き戻しが **git diff としてレビュー可能**になる粒度。
- **`fire`:** evidence 欠損時は error（実行不能）と blocked（判定結果として不合格）のどちらに寄せるか。
