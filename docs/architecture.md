...
**`hado target` と manifest:** `internal/manifest` の **`ResolveStandardPath`** と **`ApplyEvidenceScaffold`** で、解決した Readiness Standard に応じた **evidence のひな形**（空文字のフィールドなど）を manifest にマージする。将来の **`hado charge`** はここで並べたキーを自動補完する想定（`internal/charge` は未実装）。
