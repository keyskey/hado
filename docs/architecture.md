...
│
├── internal
│   ├── manifest      # HADO Manifest の読み込み・standard パス解決・evidence プレースホルダー
...
hado target --manifest hado.yaml
  ...
  解決した standard に応じ、不足している evidence キーへプレースホルダー（`PLACEHOLDER` 等）をマージする
    |
    v
developer / CI
    |
...
**`hado target` と manifest:** `internal/manifest` の **`ResolveStandardPath`** と **`ApplyEvidencePlaceholders`** で、解決した Readiness Standard に応じた **evidence のひな形**（`PLACEHOLDER` など）を manifest にマージする。将来の **`hado charge`** はここで埋めたキーを自動補完する想定（`internal/charge` は未実装）。
