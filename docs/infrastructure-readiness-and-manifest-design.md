...
CLI の **target / charge / fire** に写すと、1〜2 は主に **target**（manifest に残す照準・standard 解決・**evidence スキャフォールド**）、3 は **charge**（manifest を読み書きしながら evidence を埋める）、4 は **fire** に相当する（現状の `hado evaluate` は 1〜4 を一括で行う）。**正本は manifest** とし、`.hado/context.json` のような中間ファイルを必須にしない方針は [概要](overview.md) を参照する。詳細は [アーキテクチャ](architecture.md) の「CLI の 3 段階」を参照する。
