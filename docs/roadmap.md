...
- `hado target`（manifest に `service` / `standard` を書く; `cmd/hado`）
- `hado charge` の coverage MVP（`internal/charge`: plan / gap / apply、`cmd/hado`）
- `hado evaluate` の text / JSON 出力と終了コード 0 / 1 / 2（`cmd/hado`）

**設計上の CLI 体系:** `hado fire`（判定のみを `evaluate` から分離）や charge の SaaS 連携は [概要](overview.md) と [アーキテクチャ](architecture.md) に記載。判定は当面 `evaluate` が担う。
...
- **CLI:** `hado target`（manifest の `service` / `standard`）と `hado charge`（coverage MVP: `internal/charge` の plan/gap/apply）は実装済み。設計上の `hado fire` は [概要](overview.md)。移行期間は `hado evaluate` で判定を一括し、内部でフェーズに分解できるようにする
