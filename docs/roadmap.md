...
**設計上の CLI 体系:** `hado target`（対話で manifest に照準を書く）→ `hado charge`（manifest メタデータから evidence を自動補完）→ `hado fire`（判定のみ）の 3 段階を [概要](overview.md) と [アーキテクチャ](architecture.md) で固定した。中間生成物より manifest を正とする。実装は当面 `evaluate` のまま移行する。
...
- **CLI:** 設計上は `hado target` / `hado charge` / `hado fire` の 3 段階（[概要](overview.md)）。`target` は対話による manifest 更新、`charge` は manifest 中心の自動充填。移行期間は `hado evaluate` で一括実行を継続し、内部でフェーズに分解できるようにする
