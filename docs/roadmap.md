...
- **CLI:** `hado target`（manifest の `service` / `standard` と **evidence スキャフォールド**）は実装済み。設計上の `hado charge` / `hado fire` は [概要](overview.md)）。移行期間は `hado evaluate` で判定を一括し、内部でフェーズに分解できるようにする
